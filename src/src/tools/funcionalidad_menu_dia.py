...
class FuncionalidadMenuDia:
    ...
    def preparar_y_obtener(self):
        ...
        gen = GeneradorMenuDia(
            ServicioCargaPlatos(self._motor_gral), 
            self._motor_dia                       
        )
        
        gen.generar_si_no_existe()
        

        platos_hoy = ServicioCargaPlatos(self._motor_dia).obtener_todos_los_platos()
